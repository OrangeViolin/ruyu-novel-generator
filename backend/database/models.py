...
class NovelProject(Base):
    ...
    type = Column(String(20), default="short_story") # short_story, long_novel
    source_manuscript_id = Column(Integer, nullable=True) # 来源短篇稿件ID
    created_at = Column(DateTime, default=datetime.now)
    updated_at = Column(DateTime, default=datetime.now)
...
class LongNovelMapping(Base):
    """长文映射表 - 记录短篇每个章节如何扩展为长篇的一卷"""
    __tablename__ = 'long_novel_mappings'

    id = Column(Integer, primary_key=True, index=True)
    project_id = Column(Integer)  # 长文项目ID
    short_chapter_title = Column(String(200)) # 短篇章节标题
    short_chapter_summary = Column(Text)      # 短篇章节摘要
    volume_number = Column(Integer)           # 对应长篇卷号
    start_chapter = Column(Integer)           # 长篇起始章节号
    end_chapter = Column(Integer)             # 长篇结束章节号
...
class ImitationStep(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    project_id = Column(Integer)  # 关联的仿写项目ID
...
class BatchGenerationTask(Base):
    ...
    task_type = Column(String(50))  # short_story / long_novel / inspiration
    status = Column(String(20), default="pending")  # pending, running, completed, failed, cancelled

    # 批量配置
    total_count = Column(Integer)  # 总共要生成多少篇
    completed_count = Column(Integer, default=0)  # 已完成多少篇
    failed_count = Column(Integer, default=0)  # 失败多少篇

    # 生成参数模板（JSON格式）
    settings_template = Column(JSON)  # 设定模板
    variations = Column(JSON)  # 变化的参数列表

    # 进度信息
    current_step = Column(String(100))  # 当前执行的步骤
    progress_percentage = Column(Float, default=0.0)  # 总体进度百分比

    # 结果统计
    results = Column(JSON)  # 生成的结果列表

    # 时间记录
    created_at = Column(DateTime, default=datetime.now)
    started_at = Column(DateTime)
    completed_at = Column(DateTime)
    estimated_completion = Column(DateTime)  # 预计完成时间
...
class GenerationTask(Base):
    ...
    # 关联批量任务
    batch_id = Column(Integer)  # batch_generation_tasks.id

    # 任务信息
    project_id = Column(Integer)  # 生成的项目ID
    task_name = Column(String(200))  # 任务名称
    status = Column(String(20), default="pending")  # pending, running, completed, failed

    # 生成参数
    generation_params = Column(JSON)  # 具体的生成参数
